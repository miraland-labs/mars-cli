...
use mars::{self, state::Proof, utils::AccountDeserialize};
...
impl Miner {
    pub async fn claim(&self, beneficiary: Option<String>, amount: Option<f64>) {
        ...
        let amount = if let Some(amount) = amount {
            (amount * 10f64.powf(mars::TOKEN_DECIMALS as f64)) as u64
        } else {
        ...
        let amountf = (amount as f64) / (10f64.powf(mars::TOKEN_DECIMALS as f64));
        let cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(CU_LIMIT_CLAIM);
        let cu_price_ix = ComputeBudgetInstruction::set_compute_unit_price(self.priority_fee);
        let ix = mars::instruction::claim(pubkey, beneficiary, amount);
        ...
        {
            Ok(sig) => {
                println!("Claimed {:} MARS to account {:}", amountf, beneficiary);
                println!("{:?}", sig);
            }
            Err(err) => {
                println!("Error: {:?}", err);
